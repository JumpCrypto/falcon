/* Pornin round 3 verifier with XKCP plain64 SHAKE256 hash-to-point.
 * Same SHAKE256 stream, different backend. */
...
/* Pornin internals (`FALCON_PREFIX = falcon_inner`). */
...
/* Pornin hash-to-point with XKCP SHAKE256.  Exposed for the bench. */
void
hash_to_point_xkcp( uint16_t * out, uint8_t const * in, size_t in_len ) {
  ...
  /* Rate is even, so 16-bit rejection candidates never cross blocks. */
