/* Auto-vectorisable Falcon-512 verifier.  The NTT uses Shoup
 * multiplication with s' = floor(s*2^32/Q); the Hadamard product uses
 * one-shot Barrett, with each coefficient multiplied once. */
...
/* Keep clang from vectorising strided outer NTT loops into gather/scatter. */
#ifdef __clang__
 ...
 *     r     = v * s - q_hat * Q
 *     return r >= Q ? r - Q : r
 *   A single conditional subtract normalizes the lazy-reduction range. */
...
/* Hadamard uses Barrett; Shoup precompute would be per coefficient. */
static inline fq_t
...
/* Hash-to-point: same stream as falcon_ref_xkcp. */
...
/* AVX-512-only KTP256 hash, declared here to avoid the AVX-512 header. */
...
/* Shared verifier; only hash-to-point changes for KTP256. */
static int
...
/* Shoup C pipeline with non-standard KTP256 hash-to-point. */
int
