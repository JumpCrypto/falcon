/* AVX-512 Falcon-512 verifier using Barrett multiplication in the NTT.
 * Hash-to-point uses the vendored Pornin SHAKE backend. */
...
/* Barrett field multiplication, 4/8/16-wide u32. */
...
/* Barrett field multiplication (8-wide AVX2 u32). */
...
/* Barrett field multiplication (16-wide AVX-512 u32).
 *
 *   M       = floor(2^29 / Q) = 43687
 *   q_hat   = floor( p * M / 2^29 )
 *   r       = p - q_hat * Q
 *
 * AVX-512 uses vpmuludq on even lanes, then shifts odd lanes into place
 * for a second vpmuludq. */
...
fq_mul_v( __m512i a, __m512i b ) {
  ...
  /* r in [0,2Q); fold to [0,Q). */
  __mmask16 ge = _mm512_cmpge_epu32_mask( r, Qv );
  return _mm512_mask_sub_epi32( r, ge, r, Qv );
}

/* Forward NTT, lazy reduction:
 * u' = u + v*s, v' = u - v*s + Q; after k passes, x < (k+1)Q. */
 ...
 * u' = u + v, v' = (u - v + off_q*Q)*s.  Reduce sums when off_q reaches 8. */
...
/* Barrett AVX-512 with non-standard KTP256 hash-to-point. */
int
falcon_avx512_barrett_ktp256_crypto_sign_open( uint8_t       * m,  size_t * mlen,
                                         uint8_t const * sm, size_t   smlen,
                                         uint8_t const * pk ) {
  falcon_pubkey_t    pubk[1];
  falcon_signature_t sig [1];
  ...
  /* Preserve NIST output behavior even when the non-standard `c` fails. */
  if( m && msg_len ) memmove( m, msg, msg_len );
  if( mlen ) *mlen = msg_len;
  return ok ? 0 : -1;
}
...
/* Non-AVX-512 builds use the reference verifier. */
int
falcon_avx512_barrett_crypto_sign_open( uint8_t       * m,  size_t * mlen,
                                uint8_t const * sm, size_t   smlen,
                                uint8_t const * pk ) {
