/* Falcon-512 verifier microbench.  Tables 1/2 report verify time;
 * Table 3 reports decode + hash-to-point + NTT/norm subparts. */
...
/* Pornin internals used by subpart benches. */
...
/* fa512_hash_to_point* are declared by falcon_avx512_common.h. */
...
/* Subcomponent benches: decode, hash-to-point, NTT/norm. */
...
bench_h2p_avx512( unsigned long iter ) {
  /* +16 slack for the final masked store at c+i. */
...
emit_table3_subparts( double dec_p, double dec_a,
  ...
  /* Table baseline: falcon_ref_xkcp subpart sum. */
...
main( int argc, char ** argv ) {
  ...
  /* Reused rows are derived from the same subpart timings. */
  ...
  /* Diagnostic row: same Barrett skeleton, but eager add/sub reduction. */
